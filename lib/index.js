 ...
 */
function convertToStore (program) {
  program.isModuleUse = true
  return _convertToStore(program)
}
function _convertToStore (program) {
...
function _build (program) {
  program.importConfigFromManifest = true
  program.bundleFromCwd = true

  return setup(program)
    .then(() => manifest.bumpVersion(program))
    .then(() => program)
    .then(() => makeAppx(program))
    .then(() => sign.signAppx(program))
    .then(() => deploy(program))
...
module.exports = convertToStore
module.exports.convertToStore = convertToStore
module.exports._convertToStore = _convertToStore
module.exports.build = build
module.exports._build = _build
