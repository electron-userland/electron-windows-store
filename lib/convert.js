...
function convertWithFileCopy (program) {
  return new Promise((resolve, reject) => {
    ...
    fs.readFile(manifest, 'utf8', (err, data) => {
      ...
      if (err) {
        utils.debug(`Could not read manifest file in pre-appx location. Error: ${JSON.stringify(err)}`)
      ...
      fs.writeFile(manifest, result, 'utf8', (err) => {
        if (err) {
          utils.debug(`Could not write manifest file in pre-appx location. Error: ${JSON.stringify(err)}`)
          return reject()
        }
